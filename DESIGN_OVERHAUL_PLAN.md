...
### Als Nächstes
**Settings-Overlay `.fr-settings`** nach §2 bauen (analog `.fr-menu`/`.fr-garage`): Geister-設 + Screentone, Titel SETTINGS (Anton 132px), **01 音響 AUDIO** (Master/Music/SFX-Slider → `updateAudioSettings`; Hinweis: es gibt aktuell nur Music+SFX im SaveData, kein Master — Master entweder als Gesamt-Gain ergänzen oder vorerst weglassen/auf Music mappen), **02 表示 DISPLAY** (Mute-Toggle → `muted`, Perf-HUD-Toggle → `showPerfHud` als Pill-Switches), **03 画質 GRAPHICS** (低/中/高 → `saveData.settings.quality`, Quality-Switch möglichst live an `RendererService`). Ersetzt die alte `.settings-panel`; darf das Spiel etwas abdunkeln. Die 設-Buttons (Menu + Garage) rufen aktuell noch `toggleSettingsPanel()` → das alte Panel; nach dem Bau auf das `.fr-settings`-Overlay umstellen.

Danach: Run-HUD + Game-Over in neuer Sprache ableiten. Mobile-Feinschliff je Screen (v. a. **Garage portrait**: das Info-Panel als Bottom-Sheet verdeckt aktuell das tief stehende 3D-Auto — Panel kompakter/höher oder Portrait-Kamera anheben). Aufräumen (alte `DESIGN_VISION/SYSTEM`-CSS/DOM, ggf. Hanko-Rail/`print-frame`/`vehicleKanji` entfernen, sobald nichts mehr darauf zeigt).
