...
### Als Nächstes
**Run-HUD + Game-Over** in der neuen Bildsprache ableiten. Heute liegt im `running`-State noch die Legacy-Top-Bar (`.top-bar` mit META/金/×/圧 + Zahnrad) + bei `gameOver` die `.menu-panel` mit `.game-over-summary`. Neu bauen als `.fr-hud` (transparentes Overlay über dem Vollbild-3D-Run; META=Distanz als gebrandeter Hero-Wert, 金 Coins, × Combo, 圧 Pressure in Print-Sprache) und `.fr-gameover` (Stempel-Layout: große META-Zahl + 終 + Chips, RESTART/MAIN MENU). Legacy im jeweiligen State per CSS ausblenden (wie Menu/Garage). Werte kommen aus `runScene.getRunStats()` (siehe `updateStats()`/`updateGameOverUi()`).

Danach: Mobile-Feinschliff je Screen (v. a. **Garage portrait**: das Info-Panel als Bottom-Sheet verdeckt aktuell das tief stehende 3D-Auto — Panel kompakter/höher oder Portrait-Kamera anheben). Aufräumen (alte `DESIGN_VISION/SYSTEM`-CSS/DOM, Legacy-`.top-bar`/`.menu-panel`/`.settings-panel`/Hanko-Rail/`print-frame`/`vehicleKanji` entfernen, sobald die neuen Screens alle States abdecken und nichts mehr darauf zeigt).
